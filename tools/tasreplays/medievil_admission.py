"""Admit the independently replayed MediEvil Nymashock source, never native output.

Publication 4875M, "all chalices", a native BizHawk 2.8 Nymashock recording, replayed on the
pinned 2.8 host (the first Nymashock release) through source_control_nyma28.py with the
observer-280-generic helper (the Observation291 source adapted to the 2.8 assemblies). A 2.8
recording does not sync on a 2.9.1 core (Mega Man X4 6412M, 2026-09-16), and this movie's stick
values are not multiples of 256, so it cannot be re-encoded for a later host either. Structure
and every check are megamanx4_admission.py's; the differences are the host identities, the
2.8 launcher and lua, the two-track disc, and the 2.8 host-closure core file (shock.wbx.gz).
"""
...
MOVIE_SHA='3987101595ad6e97ae99c71a2c45db781a557c2ff2343abff20de1b95f28bc33'
CARD_SHA='78b6d4ac9ab4d23caf7e5f04f83539bf5d994cccfb0a709d14ac53d05c8e21ef'
FINAL_CARD="SaveRAM/MediEvil (USA).SaveRAM"
TITLE='MediEvil (USA) all chalices 4875M'
FIRMWARE_KEY='PSX+U'
...
FIXED={
    'MediEvil (USA) (Track 1).bin':'d522a86c154524d634a0d9b84b2048b41cafe83013a87f2fa31a2c117aabce82',
    'MediEvil (USA) (Track 2).bin':'494e8e800206856d5d4c07fe450879952dfa29071e13671c24f0f6b649a1d98e',
    'MediEvil (USA).cue':'a7f78ec53382536f1c22eeba955bedc902ac4680f2445995d3c62018a0f11c89',
    ...
    'crash41596-medievil-allchalices.bk2':MOVIE_SHA,'original.bk2':MOVIE_SHA,
    'Observation291.dll':'6a6c3725f7a177cadab1edec8951330da38ebb01cad70cbc55fd1b519b43b44a',
    ...
    'observer-build.json':'2f9c94de81f8b2568a6c5f9a82b6e785451320e99714b12c0bb227292d61e560',
}
...
def verify_identity(root,manifest,role,tail):
    ...
    expected_command=[str(app),str(Path(names['MediEvil (USA).cue']['path'])),
...
def qualify(stock,observed,output):
    ...
    result={'schema':'medievil-independent-source-v1','source_qualification':'pass',
