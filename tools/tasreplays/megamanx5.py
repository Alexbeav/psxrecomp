"""Build the exact Mega Man X5 (USA) Training X candidate after source admission.
...
import megamanx5_admission as source
...
DISC_STEM='Mega Man X5 (USA)'
EXE_SHA='f61827043c9a4f4100154cf0489a11a1cd1aa056284551fd8b1c24a613a6769b'
CONTROLLER_SHA='a7bf9fd415188e951e913d81f4de3f0f58d6f9f0f6384432255cae50bd5acfe3'
...
def verify_reference(path):
    ...
    if ref.get('schema')!='megamanx5-independent-source-v1' or ref.get('source_qualification')!='pass' or ref.get('movie_sha256')!=source.MOVIE_SHA or ref.get('original_inputs')!=source.FRAMES:
        raise ValueError('wrong independent Mega Man X5 source reference')
...
def media(cue,bios):
    ...
    if not match or lines[1:]!=['TRACK 01 MODE2/2352','INDEX 01 00:00:00']:
        raise ValueError('wrong single-track Mega Man X5 (USA) topology')
    track=(cue.parent/match[1]).resolve(strict=True)
    if not track.is_relative_to(cue.parent.resolve()) or track.stat().st_size!=582954960:
        raise ValueError('invalid Mega Man X5 data track')
    require_hash(track,source.FIXED['Mega Man X5 (USA).bin'])
    return track
...
def boot_program(track):
    ...
    with track.open('rb') as f:
        ...
        if cnf!=b'BOOT = cdrom:\\SLUS_013.34;1\r\nTCB = 4\r\nEVENT = 16\r\nSTACK = 801FFF00\r\n':
            raise ValueError('wrong boot configuration')
        data=span(*tree[BOOT])
    if hashlib.sha256(data).hexdigest()!=EXE_SHA:raise ValueError('wrong Mega Man X5 executable')
    return data
...
[runtime]
window_title = "Mega Man X5 - Training X TAS"
bios_hle = false
[video]
renderer = "software"
''',encoding='utf8')
...
def replay(args,returns=None,output=None):
    ...
    if info.get('schema')!='megamanx5-tas-candidate-v1':raise ValueError('wrong Mega Man X5 candidate')
