...
import megamanx4_admission as source
...
# (PSX_Firmware_U 0555C6FA); megamanx4_admission cross-checks it. Its kernel and
...
def verify_reference(path):
    ...
    if ref.get('schema')!='megamanx4-independent-source-v1' or ref.get('source_qualification')!='pass' or ref.get('movie_sha256')!=source.MOVIE_SHA or ref.get('original_inputs')!=source.FRAMES:
        raise ValueError('wrong independent Mega Man X4 source reference')
...
def media(cue,bios):
    require_hash(cue,source.FIXED['Mega Man X4 (USA).cue'])
    ...
    if not match or lines[1:]!=['TRACK 01 MODE2/2352','INDEX 01 00:00:00']:
        raise ValueError('wrong single-track Mega Man X4 (USA) topology')
    track=(cue.parent/match[1]).resolve(strict=True)
    if not track.is_relative_to(cue.parent.resolve()) or track.stat().st_size!=592217136:
        raise ValueError('invalid Mega Man X4 data track')
    require_hash(track,source.FIXED['Mega Man X4 (USA).bin'])
    return track
...
def boot_program(track):
    ...
    with track.open('rb') as f:
        ...
        if cnf!=b'BOOT = cdrom:\\SLUS_005.61;1\r\nTCB = 4\r\nEVENT = 16\r\nSTACK = 801FFF00\r\n':
            raise ValueError('wrong boot configuration')
        data=span(*tree[BOOT])
    if hashlib.sha256(data).hexdigest()!=EXE_SHA:raise ValueError('wrong Mega Man X4 executable')
    return data
...
[runtime]
window_title = "Mega Man X4 - Zero TAS"
bios_hle = false
[video]
renderer = "software"
''',encoding='utf8')
    ...
    native_argv=['cmake','-S',HERE,'-B',native,*common,'-DTAS_PROJECT_DIR='+str(project),
             '-DTAS_GAME_STEM='+BOOT,'-DTAS_EXE_NAME=MegaManX4-TAS','-DTAS_WINDOW_TITLE=Mega Man X4 TAS',
    ...
    info={'schema':'megamanx4-tas-candidate-v1','source_head':head,'source_tree':subprocess.check_output(['git','-C',str(ROOT),'rev-parse','HEAD^{tree}'],text=True).strip(),
...
def replay(args,returns=None,output=None):
    ...
    if info.get('schema')!='megamanx4-tas-candidate-v1':raise ValueError('wrong Mega Man X4 candidate')
