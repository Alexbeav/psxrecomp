"""Build the exact Resident Evil DC "Jill, best ending" candidate after source admission.

Publication 5963M. Same game, disc, BIOS, core and pinned BizHawk as the qualified
5920M that biohazard.py builds -- only the input log differs, 257,271 rows against
227,202 -- so the boot executable, disc stem, controller layout and runtime profile
are all that module's, unchanged. What differs is the admitted source it accepts.
...
import redc_admission as source
...
def verify_reference(path):
    ...
    if ref.get('schema')!='redc-bestending-independent-source-v1' or ref.get('source_qualification')!='pass' or ref.get('movie_sha256')!=source.MOVIE_SHA or ref.get('original_inputs')!=source.FRAMES:
...
def setup(args):
    ...
    staged_bios=project/'SCPH5500.BIN';shutil.copyfile(bios,staged_bios);require_hash(staged_bios,source.FIXED[source.FIRMWARE_NAME])
...
''',encoding='utf8')
    ...
    native_record=build_cache.stage_native(cache_root,ROOT,project,native,'ResidentEvilDC-TAS',
    ...
    build=native/'ResidentEvilDC-TAS.exe'
