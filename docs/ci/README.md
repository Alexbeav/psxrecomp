...
and validates the manifest, dependency pins, executable names, and recipe before the platform matrix.
...
**Versioning:** `workflow_dispatch` requires an explicit `X.Y.Z` version already
recorded in `VERSION` and `project-manifest.toml`. The workflow creates build
artifacts only. It has read-only repository access and cannot publish a release.
Keep exact-manifest draft creation and publication in the separate release process.
...
the emitter cache and ccache reuse matching compiled work).

Matrix: `ubuntu-24.04` (linux-x64), `windows-2022` (windows-x64),
`macos-15` (macos-arm64), `macos-15-intel` (macos-x64).

Linux compilation runs inside the pinned Ubuntu 20.04 container.
macOS applies the 11.0 deployment target to generators and setup hosts.
The setup host uses pinned static SDL source instead of a Homebrew SDL library.
The workflow applies and verifies ad-hoc signatures before packaging.
Apple notarization and downloaded-file startup remain separate qualification checks.
The exact ZIP passes payload, platform-copy, architecture, permission, dependency,
and native generator checks before upload. Setup and gameplay remain separate evidence.
The root `.release/policy.json` may record `openbios_required: true` for an
explicitly reviewed candidate. This records a payload expectation, not publication approval.
