...
set(PSXRECOMP_RUNTIME_SOURCES
    ...
    ${PSXRECOMP_ROOT}/runtime/src/savestate_admission.c
    ${PSXRECOMP_ROOT}/runtime/src/controller_policy.c
...
function(psxrecomp_add_runtime_target target)
    ...
    # Stamp the lobby pin next to the exe. Packagers must ship VERSION == this
    # stamp — rewriting VERSION after the build caused Twisted Metal 4 installs
    # where VERSION said 0.3.8 but the binary still filtered lobbies as 0.3.7.
    #
    # Single-config (Ninja/Make/MinGW): TARGET_FILE_DIR == CMAKE_BINARY_DIR.
    # Multi-config (VS/Xcode): TARGET_FILE_DIR contains the configuration.
    # Do not also generate a build-root copy on multi-config generators: the
    # Release and Debug lobby pins differ, and CMake rejects different content
    # written to one shared output. The packager reads the stamp beside the exe.
    ...
    elseif(MSVC)
        ...
        # switch to the C sources that need it instead.
        set_property(SOURCE
            ${PSXRECOMP_ROOT}/runtime/src/audio_trace.c
            ${PSXRECOMP_ROOT}/runtime/src/guest_tty.c
