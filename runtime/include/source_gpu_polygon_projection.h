...
/* Independent work estimator for untextured source-profile triangles.
...
typedef void (*SourcePolySpan)(void *,int,int,int);
static inline int source_poly_walk(const int *input_x,const int *input_y,
    ...
    for(int i=0;i<3;i++) {
        ...
        /* Coordinate wrap beyond this range is not yet a qualified draw. */
        if(v[i].x < -1024 || v[i].x > 1023 || v[i].y < -1024 || v[i].y > 1023)return -1;
    }
    ...
    for(int part=0;part<2;part++) {
        ...
        for(int y=begin;y!=end;y+=dy) {
            if(down?y>clip_bottom:y<clip_top)break;
            if(down?y<clip_top:y>clip_bottom){cost+=2;continue;}
