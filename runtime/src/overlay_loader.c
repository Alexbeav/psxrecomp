 ...
 * INCONSISTENT snapshot (resume PC at the caller's post-call point, sp still
 * mid-callee) -> a leaked stack frame that compounded across cooperative cycles
 * into a smeared jumptable index (the Ape "Checking MEMORY CARD" softlock/fatal).
 * While this depth is >0, interrupts.c still runs the guest handler, but it
 * restores the interrupted thread and defers a requested cross-thread switch to
 * a clean outer boundary (at_outermost / s_defer_switch_pending). Interrupt
 * delivery itself MUST stay enabled: a callee can legitimately wait for an
 * IRQ-backed BIOS event before returning (SF2's memory-card event pump), and
 * suppressing IRQs for the whole call unit deadlocks that wait. Incremented only
 * when the A/B toggle is on (below). */
...
static void overlay_ci_wrapper(CPUState *cpu) {
     ...
     * (s_ci_skip_unit stays for the diagnostic API; it no longer counts.) */
...
static void overlay_ci_at_wrapper(CPUState *cpu, uint32_t resume_pc) {
     ...
     * interrupt layer keeps call-unit atomicity by deferring thread switches. */
    if (overlay_irq_suppressed_now()) { s_ci_skip_supp++; return; }
    if ((i_stat & i_mask) == 0) { s_ci_skip_none++; return; }
    if ((cpu->cop0[12] & ((1u << 10) | 1u)) != ((1u << 10) | 1u)) {
        s_ci_skip_sr++;
