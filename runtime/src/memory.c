...
static void interrupt_write_mask_masked(uint32_t val, uint32_t mask, uint8_t width) {
    ...
    /* Offline Ape LOAD: BIOS clears I_MASK.7 immediately after the probe
     * SELECT abort while A6C10 is still nested. That drops the nest_irq_pulse
     * before IntRP can pop to idle / set B4E38. Hold bit7 until the nest
     * unwinds (sio_card_should_hold_imask_bit7). Netplay: helper returns 0. */
