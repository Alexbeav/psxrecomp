...
cleanup:
    for (uint32_t tag = 1; tag <= BS_SEC_ICACHE; tag++) free(sections[tag].owned);
    free(dirty_words);
    free(native_vram);
    return ok;
}
