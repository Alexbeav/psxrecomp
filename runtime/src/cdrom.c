...
void cdrom_resync_deadlines_after_restore(void)
{
    ...
    /* Disc replacement is disabled in netplay. Reset the host-side lid timer
...
int cdrom_replace_disc(const char* cue_path, const char scex[4]) {
    ...
    /* Mount transactionally, then expose a real guest-visible tray cycle.
     * The replacement stays unreadable until the timed lid-close event. */
    ...
    fprintf(stderr,
            "psxrecomp: disc replacement mounted: %s (%u sectors, %d tracks)\n",
            cue_path, iso_sector_count(iso_handle), iso_track_count(iso_handle));
    fflush(stderr);
    return 1;
}
