"""Run production display-mode transition bodies with synthetic VRAM storage.

No retail data, generated BIOS, or GL context is required. Real GL qualification
remains separate from this regression of callback timing and upload ordering.
"""
...
int main(void) {
    ...
    gp1_display_mode(0);
    fbo = 0x1357; event('P'); /* later GP0 copy restores a texture word */
    depth24_upload_policy(); /* still later CPU upload triggers old policy */
    expect(fbo == 0x1357, "post-mode-switch GP0 copy survives later upload");
    ...
    gp1_display_mode(0);
    expect(clears == 1 && fbo == 0x1357, "same-depth repeat does not clear");
...
'''.replace('POLICY', policy).replace('CALLBACK', callback).replace('DISPATCH', dispatch).replace('MODE', mode)
    ...
    if compiler.name.lower() in ('cl', 'cl.exe'):
        args = [str(compiler), '/nologo', '/std:c11', '/W4', '/WX', str(fixture), '/Fe:' + str(exe), '/Fo:' + str(output / 'transition_fixture.obj')]
    else:
