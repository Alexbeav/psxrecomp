...
source=Path(__file__).resolve().parents[2];evidence=source.parent/'evidence';root=Path(sys.argv[1]);tested='4d84a7c0aa3797da799b9085a32eb3c805e183c6'
...
for name in ['cd-reset-section-contract.json','cd-reset-successor-section-contract.json','cd-reset-domains.json','cd-pending-full-tu-check-4d84a7c0aa37.json']:files.add(root/name)
for opt in ('O0','O2'):
 identities=[]
 for stem in ['cd-reset-candidate-5c4f7374ab99-'+opt+'-observer-v1','cd-reset-candidate-4d84a7c0aa37-'+opt+'-t172-v2']:
...
files.discard(source/'runtime/tests/cd_reset_provenance.json')
...
results=json.loads((evidence/'cd-reset-results-v5.json').read_text());negative=json.loads((evidence/'cd-reset-negatives-v5.json').read_text());assert all(x['rejected'] for x in negative)
result=dict(schema='t172-cd-reset-evidence-receipt-v1',tested_commit=tested,production_blob_sha256=digest(blob),preserved_wrapper_and_sha=True,totals={k:sum(x[k] for x in results) for k in ['cases','rows','events']},negative_controls=len(negative),scope='Only process_source_reset. Actual accepted head helper preserved. Synthetic readiness and recording-only response/IRQ/SPU/XA callbacks; full27field/result/event equivalence. Fulluint64 incoming head due safe because preserved or overwritten; clock/reset_due and catch-up bounded except exact active hold valid0 phase0..2 wrap-to-zero sentinel probe. No actual IRQ/audio/XA internals, initiation/process_pending ownership, media/hardware/fullrestore/legal claim',files=entries)
output=evidence/'validation-receipt-cd-reset-v2.json'
