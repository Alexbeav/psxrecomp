...
E=Path(sys.argv[1]);D=json.loads(Path(sys.argv[2]).read_text())['fields'];rng=random.Random(920173);cases=[]
def add(s):
...
with (E/'cd-reset-holdout-v4.json').open('x') as f:json.dump(dict(schema='t172-cd-reset-experiment-v1',cases=cases),f)
print(len(cases))
