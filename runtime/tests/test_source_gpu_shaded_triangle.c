...
static void run(unsigned op,unsigned blend,unsigned mask,unsigned dither) {
 ...
 int x[]={4,28,8},y[]={4,6,20};uint32_t colors[]={0xe3942b,0x285eb0,0x76c341},words[6];
 for(unsigned i=0;i<3;i++){words[i*2]=colors[i]|(i?0:op<<24);words[i*2+1]=(unsigned)x[i]|(unsigned)y[i]<<16;}
 ...
 if(s.dispatch.kind!=SOURCE_GPU_DISPATCH_COMMAND || s.dispatch.count!=6 || s.phase || s.count)abort();int work=256-s.budget;
 ...
 int extra=-1;if(!sw_draw_source_triangle(x,y,colors,1,dither,0,0,NULL,&extra) || extra)abort();
 uint8_t hash[32];psx_sha256_compute((uint8_t*)vram,sizeof(vram),hash);
 printf("op%02x-b%u-k%u-d%u %d ",op,blend,mask,dither,work);for(unsigned i=0;i<32;i++)printf("%02x",hash[i]);puts("");
}
int main(void){for(unsigned op=0x30;op<=0x33;op++)for(unsigned b=0;b<4;b++)for(unsigned k=0;k<4;k++)for(unsigned d=0;d<2;d++)run(op,b,k,d);return 0;}
