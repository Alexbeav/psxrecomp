...
from cd_head_model import expected_rows
def read_check(matrix,item,commit):
 cases=json.loads(Path(matrix).read_text())['cases'];count=0
 with Path(item['trace']).open() as f:
  meta=json.loads(next(f))['metadata'];identity=json.loads(Path(item['identity']).read_text())
  require(meta['schema']=='t77-cd-head-observations-v1','schema');require(meta['source']==identity and identity['commit']==commit,'source identity')
  ...
  for a,e in itertools.zip_longest((json.loads(l) for l in f),expected_rows(matrix)):
   require(a is not None and e is not None,'row count');require(a==e,f'{e["case_id"]} full state');count+=1
 require(meta['cases']==len(cases) and meta.get('observations',meta.get('cases'))==count,'counts');return meta,dict(cases=len(cases),rows=count)
