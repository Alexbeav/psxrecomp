...
from cdda_service_model import expected_rows
def read_check(matrix,item,commit):
 cases=json.loads(Path(matrix).read_text())['cases'];count=events=fatal=frames=0
 with Path(item['trace']).open() as f:
  meta=json.loads(next(f))['metadata'];identity=json.loads(Path(item['identity']).read_text())
  require(meta['schema']=='t77-cdda-service-observations-v1','schema');require(meta['source']==identity and identity['commit']==commit,'source identity')
  ...
  for a,e in itertools.zip_longest((json.loads(l) for l in f),expected_rows(matrix)):
   require(a is not None and e is not None,'row count');require(a==e,f'{e["case_id"]}:{e["step"]} full state/events');count+=1;events+=len(a['events']);fatal+=int(a['termination']!=0);frames+=sum(e['args'][0] for e in a['events'] if e['kind']==10)
 require(meta['cases']==len(cases) and meta['observations']==count,'counts');return meta,dict(cases=len(cases),rows=count,events=events,fatal=fatal,frames=frames)
