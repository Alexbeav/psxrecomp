...
int main(void) {
    ...
    /* Vengeance observed geometry: 320 RGB pixels, y28/h192, two pages.
     * Keep prior stills intact; only the active movie's unused rows are black.
     */
    uint32_t movie[320];
    test_streaming = 0;
    di.display_y = 256;
    gpu_depth24_present_row(&di, 0, movie, 320);
    gpu_depth24_present_row(&di, 239, movie, 320);
    assert(movie[319] == 0xff121212);
    test_streaming = 1;
    gpu_depth24_present_row(&di, 0, movie, 320);
    for (unsigned page = 0; page <= 256; page += 256) {
        for (unsigned column = 0; column < 480; column += 24) {
            vram_write_x = column; vram_write_y = page + 28;
            vram_write_w = 24; vram_write_h = 192;
            gp0_commit_cpu_to_vram();
        }
        di.display_y = page;
        gpu_depth24_present_row(&di, 27, movie, 320);
        assert(movie[319] == 0xff000000);
        gpu_depth24_present_row(&di, 28, movie, 320);
        assert(movie[319] == 0xff121212);
        gpu_depth24_present_row(&di, 219, movie, 320);
        assert(movie[319] == 0xff121212);
        gpu_depth24_present_row(&di, 220, movie, 320);
        assert(movie[319] == 0xff000000);
    }
    test_streaming = 0;
    gpu_depth24_present_row(&di, 0, movie, 320);
