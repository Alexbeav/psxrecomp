...
int main(void) {
    ...
    /* The first upload must survive even if no present saw the MDEC edge. */
    depth24_reset_rows();
    vram_write_y = 20; vram_write_h = 1;
    gp0_commit_cpu_to_vram();
    gpu_depth24_present_row(&di, 20, out, 4);
    assert(out[0] == 0xff121212);
    /* Restore is deterministic regardless of previous host streaming history. */
    for (int prior = 0; prior < 2; prior++) {
        s_d24_streaming = prior;
        memset(s_d24_rows, 1, sizeof(s_d24_rows));
