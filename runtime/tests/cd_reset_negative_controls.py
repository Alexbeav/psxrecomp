"""Reject altered reset result, state, event order and fixture domains."""
import copy,json,subprocess,sys,tempfile
from pathlib import Path
from validate_cd_reset import compare
p=json.loads(Path(sys.argv[1]).read_text());job=copy.deepcopy(p['jobs'][4]);results=[]
scratch=Path(__file__).resolve().parents[2]/'_scratch';scratch.mkdir(exist_ok=True)
with tempfile.TemporaryDirectory(dir=scratch,prefix='reset-negative-') as temp:
 ...
 check('expected-commit',lambda a,p,j:p.__setitem__('tested_commit','wrong'));check('rows',lambda a,p,j:a.pop());check('case-id',lambda a,p,j:a[-1].__setitem__('case_id','wrong'));check('result',lambda a,p,j:a[-1].__setitem__('result',1))
 for key in original[-1]['state']:check(key,lambda a,p,j,k=key:a[-1]['state'].__setitem__(k,a[-1]['state'][k]+1))
 for kind in range(1,11):check('args-'+str(kind),lambda a,p,j,k=kind:event(a,k)['args'].__setitem__(0,event(a,k)['args'][0]+1))
 for kind in range(1,11):check('event-state-'+str(kind),lambda a,p,j,k=kind:event(a,k)['state'].__setitem__('phase',99))
 check('order',lambda a,p,j:next(r for r in a[1:] if len(r['events'])>2)['events'].reverse())
 valid=json.loads(Path(job['matrix']).read_text())['cases'][-1]['state']
 changes=[dict(phase=-2147483649),dict(phase=2147483648),dict(lba=-2147483649),dict(lba=2147483648),dict(mode=256),dict(clock=-1),dict(due=2**64),dict(reset_due=2**64-903168),dict(source_clock=2**31),dict(reading=2**31),dict(hold=-2147483649),dict(hold=1,target=2147483646),dict(seek_min=-1),dict(seek_sec=256),dict(muted=256),dict(ready=2**31),dict(source_clock=1,drive=1,reset_due=1,clock=4096*225792+1)]
 for n,delta in enumerate(changes):
  inp=root/f'invalid-{n}.json';inp.write_text(json.dumps(dict(schema='t172-cd-reset-experiment-v1',cases=[dict(id='invalid',state=dict(valid,**delta))])))
