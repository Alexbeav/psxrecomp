"""Independent reset state and callback verification."""
import copy,hashlib,itertools,json,subprocess,sys
from pathlib import Path
def require(ok,why):
 if not ok:raise ValueError(why)
def digest(p):return hashlib.sha256(Path(p).read_bytes()).hexdigest()
from cd_reset_model import expected_rows
def read_check(matrix,item,commit):
 ...
 with Path(item['trace']).open() as f:
  ...
  require(identity['head_sha256']==identity['selected_header_sha256'],'owned fragment identity');require(meta['schema']=='t77-cd-reset-observations-v1','schema');require(meta['source']==identity and identity['commit']==commit,'source identity')
 ...
 require(meta['cases']==len(cases) and meta.get('observations',meta.get('cases'))==count,'counts');return meta,dict(cases=len(cases),rows=count,events=events)
