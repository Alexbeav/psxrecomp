...
from cd_toc_model import expected_rows
def read_check(matrix,item,commit):
 ...
 with Path(item['trace']).open() as f:
  ...
  require(identity['head_sha256']==identity['selected_header_sha256'],'owned fragment identity');require(meta['schema']=='t77-cd-toc-observations-v1','schema');require(meta['source']==identity and identity['commit']==commit,'source identity')
