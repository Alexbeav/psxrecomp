...
static uint16_t expected_pixel(uint16_t back,uint16_t texel,unsigned color,
    ...
    for(unsigned c=0;c<3;c++) {
        int fore=(texel>>(5*c))&31;
        if(!raw)fore=(fore*((color>>(8*c))&255))>>7;
        if(fore>31)fore=31;
        int bg=(back>>(5*c))&31;
        if(texel&0x8000) {
...
static void pixels(unsigned mode,unsigned blend,unsigned mask,unsigned color,unsigned raw) {
    ...
    uint32_t words[]={((0x66u+raw)<<24)|color,0x00040004,300u<<22,0x00010004};
    SourceGPUBlock b={0};b.words=words;b.x=4;b.y=4;b.clip_right=1023;b.clip_bottom=511;
    b.draw_mode=16|(mode<<7)|(blend<<5);
    int extra=-1;
    check(sw_draw_source_block(&b,&extra),"textured blend software renderer admission");
    check(extra==(mode==0?20:mode==1?260:4),"full CLUT/texture cache charge, including transparent or masked pixels");
    for(unsigned i=0;i<4;i++)check(vram[4*1024+4+i]==expected_pixel(original[i],texels[i],color,blend,mask,raw),
    ...
    if(dump_vram) {
        char name[80];snprintf(name,sizeof(name),"op%02x-m%u-b%u-k%u-c%06x.bin",0x66+raw,mode,blend,mask,color);
