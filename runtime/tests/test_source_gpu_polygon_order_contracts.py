...
f=json.loads(Path(__file__).with_name('source_gpu_polygon_order_fixtures.json').read_text())
...
rows=[x.split() for x in r.stdout.splitlines()];assert len(rows)==len(f['cases'])==18
...
print('18 stock full VRAM images and36 original-source command-work values PASS')
