...
from validate_cd_toc import compare
p=json.loads(Path(sys.argv[1]).read_text());job=copy.deepcopy(p['jobs'][1]);results=[]
...
with tempfile.TemporaryDirectory(dir=scratch,prefix='toc-negative-') as temp:
 ...
 changes=[dict(read_min=-65536),dict(read_sec=65536),dict(read_sect=65536),dict(jitter=25000),dict(lba=-2147483649),dict(lba=2147483648),dict(mode=256),dict(clock=-1),dict(due=2**64),dict(source_clock=2**31),dict(reading=2**31),dict(hold=-2147483649),dict(target=2147483648)]
 for n,delta in enumerate(changes):
  inp=root/f'invalid-{n}.json';inp.write_text(json.dumps(dict(schema='t172-cd-toc-experiment-v1',cases=[dict(id='invalid',state=dict(valid,**delta))])))
