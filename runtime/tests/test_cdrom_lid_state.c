/* Validate the guest-visible CD tray interval used for live disc changes. */
#include "cdrom_lid.h"
...
int main(void)
{
    ...
    CHECK(lid.physical_open, "replacement opens the physical lid");
    CHECK(lid.shell_open_latched, "replacement latches ShellOpen");
    CHECK(!cdrom_lid_media_ready(&lid, 1),
          "replacement image stays hidden while the lid is open");
    ...
    CHECK(cdrom_lid_media_ready(&lid, 1),
          "replacement becomes readable after the lid closes");
